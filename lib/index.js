...
function buildNodeHistory(node, history) {
  ...
  if (node && node.key && jsonSchemaKeywords.indexOf(node.key.toLowerCase()) === -1) {
...
function derefSchema(schema, options, state) {
  ...
  return traverse(schema).map(function (node) {
    if (node && node['$ref'] && typeof node['$ref'] === 'string') {
      ...
      if ((refType === 'local' && nodeHistory.indexOf(finalRef) >= 0) || (state.history.indexOf(refVal) >= 0)) {
        state.circular = true;
